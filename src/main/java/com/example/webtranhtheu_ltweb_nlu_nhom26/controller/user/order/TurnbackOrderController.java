...
@WebServlet(name = "TurnbackOrderController", value = "/turnback-orders")
public class TurnbackOrderController extends HttpServlet {
    public ProductService productService = new ProductService();
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        HttpSession session = request.getSession();
        User account = (User) session.getAttribute("account");
        List<Order> turnback= OrderService.getListOrderByUserAndStatus(account.getId(),6);
        turnback.forEach(order -> {
