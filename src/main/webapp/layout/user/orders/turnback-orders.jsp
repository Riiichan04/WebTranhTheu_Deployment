    ...
    <c:if test="${not empty turnback}">
        <c:forEach var="order" items="${turnback}">
            ...
            <div id="${order.id}" class="card container">
                ...
                <div class="row py-2">
                    <div class="col-9"></div>
                    <button type="button" class="turnback_details col-3" onclick="getDetailsForm(${order.id})">Chi tiết</button>
                </div>
            </div>
        </c:forEach>
    </c:if>
